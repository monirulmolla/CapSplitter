...
export class BillService {
    ...
    addBill(bill: Bill) {
        ...
        const token = localStorage.getItem('token')
            ...
            : '';
        return this.http.post('http://localhost:3000/bill' + token, body, {headers: headers}).map(
            //TODO handle response and error here
            response => this.onAddBillSuccess (response))
            .catch(error => {
    ...
    getBills() {
        ...
        const token = localStorage.getItem('token')
            ? '?token=' + localStorage.getItem('token')
            : '';
        return this.http.get('http://localhost:3000/bill'+ token, {headers: headers}).map(
            //TODO handle response and error here
